 ...
 * - whitelist filtrů: [ 'id', 'user_id', 'consent_type', 'version', 'granted', 'granted_at', 'source', 'meta' ]
 * - whitelist pro LIKE hledání: [ 'consent_type', 'version', 'source' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'user_id', 'consent_type', 'version', 'granted', 'granted_at', 'source', 'meta' ], true);
        ...
        if ($this->search !== null) {
            $searchCols = [ 'consent_type', 'version', 'source' ];
            $likeParts = [];
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'user_id', 'consent_type', 'version', 'granted', 'granted_at', 'source', 'meta' ], true);
            $parts = [];
