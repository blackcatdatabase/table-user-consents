...
use BlackCat\Database\Packages\UserConsents\Repository\UserConsentRepositoryInterface;
...
class UserConsentRepository implements UserConsentRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'user_id', 'consent_type', 'version' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\UserConsents\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'granted', 'granted_at', 'source', 'meta' ]);
          return count($rows);
        }
    ...
    public function getByUserIdAndConsentTypeAndVersion(int $userId, string $consentType, string $version, bool $asDto = false): array|\BlackCat\Database\Packages\UserConsents\Dto\UserConsentDto|null {
        return $this->getByUnique([ 'user_id' => $userId, 'consent_type' => $consentType, 'version' => $version ], $asDto);
    }
    ...
    public function getIdByUserIdAndConsentTypeAndVersion(int $userId, string $consentType, string $version) {
        $row = $this->getByUserIdAndConsentTypeAndVersion($userId, $consentType, $version, false);
