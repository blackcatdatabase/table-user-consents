...
namespace BlackCat\Database\Packages\UserConsents\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\UserConsents\Definitions;
use BlackCat\Database\Packages\UserConsents\Criteria;
use BlackCat\Database\Packages\UserConsents\Dto\UserConsentDto as Dto;
use BlackCat\Database\Packages\UserConsents\Mapper\UserConsentDtoMapper as RowMapper;
...
final class UserConsentRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\UserConsents\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'granted', 'granted_at', 'source', 'meta' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'user_id', 'consent_type', 'version' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\UserConsents\Definitions::table());
            return $bulk->upsertMany($rows, [ 'user_id', 'consent_type', 'version' ], [ 'granted', 'granted_at', 'source', 'meta' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\UserConsents\Dto\UserConsentDto|null */
    public function getByUserIdAndConsentTypeAndVersion(int $userId, string $consentType, string $version, bool $asDto = false): array|\BlackCat\Database\Packages\UserConsents\Dto\UserConsentDto|null {
        $row = $this->getByUnique([ 'user_id' => $userId, 'consent_type' => $consentType, 'version' => $version ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\UserConsents\Mapper\UserConsentDtoMapper::fromRow($row);
    }
    public function existsByUserIdAndConsentTypeAndVersion(int $userId, string $consentType, string $version): bool {
        $where = 't.' . Ident::q($this->db, 'user_id') . ' = :uniq_user_id' . ' AND ' . 't.' . Ident::q($this->db, 'consent_type') . ' = :uniq_consent_type' . ' AND ' . 't.' . Ident::q($this->db, 'version') . ' = :uniq_version';
        return $this->exists($where, [ 'uniq_user_id' => $userId, 'uniq_consent_type' => $consentType, 'uniq_version' => $version ]);
    }
    /** @return int|string|null */
    public function getIdByUserIdAndConsentTypeAndVersion(int $userId, string $consentType, string $version) {
        $row = $this->getByUserIdAndConsentTypeAndVersion($userId, $consentType, $version);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\UserConsents\Dto\UserConsentDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\UserConsents\Dto\UserConsentDto|null {
