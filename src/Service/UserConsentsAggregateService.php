 ...
 */
final class UserConsentsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private UserConsentRepository $userConsentRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'user_consents' => $this->userConsentRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\UserConsents\ModuleException("Record {$idStr} not found for locking.");
            }
